...
def collect_rows(*, suite: str, spaced: bool, model_type: str, metric: str = "l2") -> list[dict]:
    out: list[dict] = []
    for regime in validation_suite_curriculum(suite):
...
def plot_curriculum(
    ...
    *,
    ...
    ax.set_title(
        f"{model_type} {repr_label} sensitivity across {suite_title.lower()} "
...
def main() -> None:
    ...
    rows = collect_rows(
        suite=args.suite, spaced=spaced, model_type=args.model_type, metric="l2",
    )
    out_dir = validation_viz_dir(
        suite=args.suite, spaced=spaced, model_type=args.model_type, kind="dfa_sensitivity",
    )
    ...
    for regime in validation_suite_curriculum(args.suite):
        exp = _curriculum_experiment(regime, spaced=spaced)
