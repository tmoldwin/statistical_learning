"""Closed-loop trajectories for the micro curriculum (regimes × inits)."""
...
from experiment import (
    ...
    validation_suite_init_seeds,
    validation_suite_labels,
    validation_suite_title,
    validation_trajectory_panel_kind,
    validation_trajectory_row_label,
    validation_viz_dir,
)
...
REGIME_LABELS = None  # set per suite in main()
...
def _completed_word_end_indices(word_at_step: list[str], vocab: set[str]) -> list[int]:
    """Indices where a vocabulary word finishes in the rollout."""
    ends: list[int] = []
...
def _teacher_forced_next_char_accuracy(model: dict, text: str, output_probs: np.ndarray) -> float:
    ...
    n = min(len(text) - 1, len(output_probs))
    if n <= 0:
        return float("nan")
    correct = sum(
        int(np.argmax(output_probs[i])) == char_to_idx[text[i + 1]]
        for i in range(n)
...
def _trajectory_panel(
    ...
    *,
    ...
    ckpt = checkpoint_path(exp, model_type, seed=init_seed)
    if not ckpt.is_file():
    ...
    if len(z) >= 2:
        ...
        end_labels = _sparse_word_end_labels(word_at_step, vocab, len(z))
        _annotate_trajectory_labels(
            ax, z, end_labels,
...
def _write_figure(
    *,
    ...
    has_any = any(
        checkpoint_path(exp, model_type, seed=s).is_file()
        for exp in exps
        for s in init_seeds
    )
    if not has_any:
    ...
    for row, init_seed in enumerate(init_seeds):
        for col, (regime, exp) in enumerate(zip(regimes, exps, strict=True)):
            ...
            if row == 0:
                tag = REGIME_LABELS.get(regime, regime)
                ax.set_title(f"({tag})", fontsize=8)
            if col == 0:
                row_label = validation_trajectory_row_label(suite, init_seed)
                if row_label:
                    if n_components == 3:
                        ax.text2D(
                            -0.12, 0.5, row_label,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="rollout length (default: 3 vocabulary cycles per panel)",
    )
    parser.add_argument(
        "--seed-char",
        default=None,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="weight-init seeds (rows); default depends on --suite",
    )
    parser.add_argument(
        "--no-word-space",
        action="store_true",
    ...
    parser.add_argument(
        ...
        choices=("2", "3", "both"),
        default="both",
        help="output 2D, 3D, or both (default: both)",
    )
    parser.add_argument(
        "--suite",
        choices=list(VALIDATION_SUITES),
        default="micro",
        help="validation suite to plot (default: micro)",
    )
    parser.add_argument(
        "--model-type",
        default="rnn",
    ...
    REGIME_LABELS = validation_suite_labels(args.suite)
    curriculum = validation_suite_curriculum(args.suite)
    suite_title = validation_suite_title(args.suite)

    exps = list(curriculum) if args.no_word_space else [
        spaced_experiment_name(r) for r in curriculum
    ]
    regimes = list(curriculum)
    init_seeds = (
        list(args.seeds)
        if args.seeds is not None
        else list(validation_suite_init_seeds(args.suite))
    )
    ...
    panel_kind = validation_trajectory_panel_kind(args.suite)
    base_title = (
        f"{suite_title}: trajectories by {panel_kind} "
        f"({spacing}, {args.model_type} {repr_label}"
    )
    out_dir = validation_viz_dir(
        suite=args.suite, spaced=spaced, model_type=args.model_type, kind="trajectories",
    )
    stem = f"by_{panel_kind}"
    ...
    if args.dims in ("3", "both"):
        _write_figure(
            ...
            seed_char=args.seed_char,
            n_components=3,
            out_path=out_dir / f"{stem}_3d.png",
            suptitle=f"{base_title}, 3D)",
            suite=args.suite,
        )
