...
PANEL_TITLES: dict[str, str] = {
    ...
    "sixteen_word_overlap": "-at/-et/-an/-ar",
    "sixteen_word_ig_um": "-ig/-og/-um",
}
...
def _curriculum_experiments(*, suite: str, spaced: bool) -> list[str]:
    curriculum = validation_suite_curriculum(suite)
    if spaced:
        return [spaced_experiment_name(regime) for regime in curriculum]
...
def plot_curriculum_heatmaps(
    ...
    *,
    ...
    fig.suptitle(
        f"Unit selectivity across {suite_title.lower()} ({spacing})",
        fontsize=11,
        y=1.02,
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="micro",
        help="validation suite to analyze (default: micro)",
    )
    parser.add_argument(
        "--model-type",
        default="rnn",
    ...
    items: list[ExperimentSelectivity] = []
    for exp in _curriculum_experiments(suite=args.suite, spaced=spaced):
        row = load_experiment_selectivity(exp, model_type=args.model_type)
        if row is not None:
            items.append(row)

    if not items:
        print(f"no {args.model_type} checkpoints found for {args.suite} suite")
        return

    out_dir = validation_viz_dir(
        suite=args.suite, spaced=spaced, model_type=args.model_type, kind="unit_selectivity",
    )
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    plot_curriculum_heatmaps(
        items, out_dir / "heatmap.png", spaced=spaced, suite_title=suite_title,
    )
