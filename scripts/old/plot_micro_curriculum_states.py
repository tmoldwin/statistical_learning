...
def _collect_states_panel(
    ...
    *,
    ...
    ckpt = checkpoint_path(exp, model_type, seed=seed)
    if not ckpt.is_file():
        return None
...
def _write_feature_figure(
    *,
    ...
    has_any = any(
        checkpoint_path(exp, model_type, seed=seed).is_file()
        for exp in exps
        for seed in seeds
    )
    if not has_any:
    ...
    for row, seed in enumerate(seeds):
        for col, (regime, exp) in enumerate(zip(regimes, exps, strict=True)):
            ax = axes[row, col]
            if not checkpoint_path(exp, model_type, seed=seed).is_file():
                ax.text(
                    0.5, 0.5, f"missing\nseed {seed}",
            ...
            if col == 0:
                row_label = validation_trajectory_row_label(suite, seed)
                if row_label:
                    ax.text(
                        -0.28, 0.5, row_label,
    ...
    fig.suptitle(
        f"{suite_title}: per-letter states by {feat_title} "
...
def _write_experiments_by_feature_figure(
    *,
    ...
    has_any = any(
        checkpoint_path(exp, model_type, seed=_seed_for_experiment_row(i, seeds)).is_file()
    ...
    for row, (regime, exp) in enumerate(zip(regimes, exps, strict=True)):
        ...
        words = REGIMES[regime]
        tag = REGIME_LABELS.get(regime, regime)
        missing = panel is None or not checkpoint_path(exp, model_type, seed=seed).is_file()
        ...
        for col, feature in enumerate(color_features):
            ...
            if col == 0:
                row_label = validation_trajectory_row_label(suite, seed)
                ylabel = f"({tag})" if row_label is None else f"{row_label}\n({tag})"
                ax.text(
                    -0.32, 0.5,
                    ylabel,
                    transform=ax.transAxes,
    ...
    repr_label = micro_curriculum_repr_label(model_type)
    fig.suptitle(
        f"{suite_title}: per-letter states — experiments × features "
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="initialization seeds (rows); default depends on --suite",
    )
    parser.add_argument(
        "--suite",
    ...
    out_dir = validation_viz_dir(
        suite=args.suite, spaced=spaced, model_type=args.model_type, kind="states",
    )
    ...
    for feature in features:
        _write_feature_figure(
            ...
            suite=args.suite,
            out_path=out_dir / f"{feature}_by_{panel_kind}.png",
        )
