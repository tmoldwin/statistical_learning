...
## Session ? all-runs motif homogenization board

Extended `scripts/r43_motif_figs.py --only all-runs-over-learning` (+ `viz/compare/mixed_dfa_motif_all_runs.py`, `compute_weight_colored_hl_motif_counts` in `viz/weight_structure.py`).

- 50 Dale runs, 8 subsampled learning snaps each, first?last fold per signed triad class (start?20).
- Scatter colored by **DFA states** (viridis); pooled start vs fold R??0.11 over 5400 points (108 classes ? 50 runs). Weaker than r43 alone (0.75) because runs stack vertically without run demeaning ? within-run compression still visible as negative slope bands.
- Budget panels: median across runs vs normalized progress (edges ?0.71, triads ?0.49, sd log-count 2.12?2.39).

![all runs motif board](../../experiments/comparisons/mixed_vocab_dfa_ns/trajectories/mixed_dfa_motif_counts_raw_over_learning.png)
